...
public class GMusicGUI {
	...
	public GMusicGUI(UUID UUID, MenuType Type) {
		uuid = UUID;
		type = Type;
		playSettings = GPM.getPlaySettingsService().getPlaySettings(uuid);
		inventory = Bukkit.createInventory(new InventoryHolder() {

			@Override
			public @NotNull Inventory getInventory() { return inventory; }

		}, 6 * 9, GPM.getMessageService().getMessage("MusicGUI.title"));
		...
		listener = new Listener() {
			...
			public void ICliE(InventoryClickEvent Event) {
				if(Event.getInventory().equals(inventory)) {
					...
					if(GPM.getVersionManager().getInventoryFromView(Event.getView(), "getTopInventory").equals(Event.getClickedInventory())) {
						...
						if(itemStack != null && itemStack.hasItemMeta() && itemStack.getItemMeta().hasLocalizedName()) {
							...
							if(clickName.startsWith("!")) setPage(Integer.parseInt(clickName.replace("!", "")));
								...
								GSong S = GPM.getSongManager().getSongById(id);
	...
	public void setPauseResumeBar() {
		...
		GPlayState songSettings = GPM.getPlaySongManager().getSongSettings(uuid);
	...
	private int getMaxPageSize(List<GSong> Songs) {
		int i = Songs.size();
